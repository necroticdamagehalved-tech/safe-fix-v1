...
export function setFirstGenTag(id){
  ...
  if(state.choices.tags.includes(id)){
    state.choices.tags = state.choices.tags.filter(t=>t!==id);
  }
}
...
function renderPerkTile(perk, overspent, canPick){
  ...
  pick.textContent =
    pr.owned ? (pr.hasNext?"Rank":"Maxed") : "Pick";
